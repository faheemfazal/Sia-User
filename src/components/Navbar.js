  ...
  return (
    <div className="sticky top-0 z-50 bg-white">
      <div className="w-full h-20 flex justify-between md:px-14 px-4 font-sans font-semibold items-center">
        ...
        <div className="flex gap-5 overflow-auto md:visible md:static absolute md:mx-10">
          <div className="text-[#747d88] hover:text-[#63247d]" onClick={() => navigate("/home")}>Home</div>
          <div className="nav-item nav-link text-[#747d88] hover:text-[#63247d]" onClick={() => { if (token) { navigate('/order-Details') } else { navigate('/login-signup') } }}>Order Detail</div>
        ...
        <div className="flex m-3 mr-0 gap-2">
          ...
          {!token && <div className="p-2 visible md:invisible md:absolute static" onClick={() => { navigate("/login-signup") }}>
            <FaUser className="text-[#63247d] text-2xl md:text-3xl visible md:invisible md:absolute static" />
          </div>}
          {!token && <div className="p-2 bg-black text-white text-sm md:text-md rounded-xl whitespace-nowrap invisible md:visible md:static absolute" onClick={() => { navigate("/login-signup") }}>
            Login/sign up
          </div>}
                ...
                <div className="bg-white shadow-md rounded-md p-2 absolute top-16 right-2 left-2">
                  <div className="bg-white">
                    <div className="flex flex-col items-start text-xl md:text-2xl font-normal gap-2 text-[#747d88]">
                      ...
                      <div className="nav-link hover:text-[#63247d]" onClick={() => { if (token) { navigate('/order-Details') } else { navigate('/login-signup') } }}>Order Detail</div>
      ...
      <div className="w-full h-10 flex justify-between md:px-14 px-4 font-sans font-semibold items-center visible md:invisible md:absolute static">
